...
create_dir(c("./Results_IBD/LOESS"))

##9. CHOOSE A NAME FOR THE PROJECT. MUST BE THE SAME ONE THAN OTHER STEPS:
...
##### 2. TRANSFORM GENETIC DISTANCES IN MATRIX AND DIST OBJECTS ------
#A. Create a list with all Relatedness distance dataframes
Related_files = list(REL1, REL2, REL3)
...
###### 4. PERFORM THE LOESS -----
#IT WILL TAKE A LONG TIME DEPENDING ON NUMBER OF SAMPLES AND SNPS.
...
#B. Create a plot
LOESS_PLOT = ggplot(dd_SN, aes(x=cov,y=obs)) + #change for cluster here
  geom_line(size=1.2) +
  geom_ribbon(aes(ymax = uci, ymin = lci), alpha = 0.3)+
  theme_minimal() + 
  geom_hline(yintercept = mean(ot_SN$disto), lty = 3) + #change for cluster here
...
plot(LOESS_PLOT)
#save as pdf
pdf(paste0("./Results_IBD/LOESS/LOESS_ALL_TESS_", project_name, ".pdf"), family="Helvetica", onefile = F) #change for cluster here
plot.new()
plot(LOESS_PLOT)
dev.off()
